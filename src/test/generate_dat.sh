...
INCOMPLETE_EMERGE="emerge_fail.txt"
INCOMPLETE_PDEPA="pdepa_fail.txt"
...
[ -e "${INCOMPLETE_EMERGE}" ] && rm "${INCOMPLETE_EMERGE}"
[ -e "${INCOMPLETE_PDEPA}" ] && rm "${INCOMPLETE_PDEPA}"
touch "${INCOMPLETE_EMERGE}"
touch "${INCOMPLETE_PDEPA}"
...
for i in $LIST; do
  ...
  if [ "$EFAIL" = "0" ]; then
    if grep -q 'success' "bench/$i/pdepa.res"; then
      echo "$i" >> "${INCOMPLETE_EMERGE}"
    fi
  elif grep -q 'fail' "bench/$i/pdepa.res"; then
    echo "$i" >> "${INCOMPLETE_PDEPA}"
  fi
done
