...
from gzoumlib.gzoumUtils import container, emptyset
...
class dep_solver(object):
  ...
  kind = container(package=0, group=1)
  ...
  def _step(self):
    ...
    if(len(new_packages) is not 0):
      ...
      for cpv in new_packages:
        ...
        if((package._group is not None) and (package._group not in self._loaded_els)):
          constraint = self._z3_translator.visit(self._repo.get_group(package._group))
          if(not self._manage_constraint(constraint, package._group, dep_solver.kind.group)): return False
    return True
  ...
  def _conclude(self):
    if(self._solution is None):
      ...
      self._conflict = (
        ...
        frozenset(k for k,v in self._loaded_map.items() if((v[0] is dep_solver.kind.group) and (v[1] in unsat)))
      )
  ...
  def _manage_constraint(self, constraint, name, kind):
    if(constraint is False):
      ...
      elif(kind is dep_solver.kind.group): self._conflict = (emptyset, frozenset((name,)))
      return False
  ...
  def model(self):
    ...
    for f in self._solution:
      if(repository.feature_is_iuse(f)):
        cpv, use = repository.feature_deconstruct(f)
        puse = solution_use_tmp.get(cpv)
        if(puse is None):
          puse = []
          solution_use_tmp[cpv] = puse
        puse.append(use)
      else: solution_packages.add(f)
    #
    solution_use = {} # BUG: if exploration is not on all iuse, but only on positive or negative... Need to change that
    for cpv, puse in solution_use_tmp.items():
      package = self._repo.get_package(cpv)
      iuses = frozenset(package._use_map.keys()).difference(package._fixed_product.keys())
      solution_use[cpv] = {use:(use in puse) for use in iuses }
    #
    solution_mask = {}
    ...
    #
    solution_uninstall = self._repo.get_installed_packages().difference(solution_packages)
    #
  ...
  @staticmethod
  def _get_package_from_sol(solution): return { f for f in solution if(repository.feature_is_package(f)) }
...
def generate_installation_files(solution_packages, solution_use, solution_mask, solution_uninstall):
  ...
  with open(path_mask_configuration, 'w') as f:
    ...
    f.write("# Do not update, any modification on this file will will overwritten by the tool\n")
    f.write("\n")
    for cpv, masking in solution_mask:
